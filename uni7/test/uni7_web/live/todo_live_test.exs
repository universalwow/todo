defmodule Uni7Web.TodoLiveTest do
  ...
  describe "Index" do
    ...
    test "saves new todo", %{conn: conn} do
      ...
      assert index_live
             |> form("#quick-todo-form", todo: @invalid_attrs)
             |> render_change() =~ "can&#39;t be blank"

      html =
        index_live
        |> form("#quick-todo-form", todo: @create_attrs)
        |> render_submit()

      assert html =~ "some title"
    end

    test "toggles todo completion in listing", %{conn: conn, todo: todo} do
      {:ok, index_live, _html} = live(conn, ~p"/todos")

      assert has_element?(index_live, "input[phx-value-id='#{todo.id}'][checked]")

      index_live
      |> element("input[phx-value-id='#{todo.id}']")
      |> render_click()

      refute has_element?(index_live, "input[phx-value-id='#{todo.id}'][checked]")
    end
